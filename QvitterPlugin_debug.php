...
// 		$settings['customwelcometext']['en'] = '<h1>Welcome to Quitter.se – a federated microblog common!</h1><p>Etc etc...</p>';
    ...
    public function initialize()
    {
        ...
        error_log("\n\n\ni'm in initialized!");
        error_log('$qvitter_enabled_by_user = '.$qvitter_enabled_by_user);
        error_log('$qvitter_disabled_by_user = '.$qvitter_disabled_by_user);
        if(is_null($scoped)) {
            error_log('$scoped = null');
        } else {
            error_log('$scoped != null');
        }
        if(static::settings('enabledbydefault') === true) {
            error_log('static::settings(\'enabledbydefault\') === true');
        } else {
            error_log('static::settings(\'enabledbydefault\') === false');
        }
        if($this->hijack_ui === true) {
            error_log('1) $this->hijack_ui = true');
        } else {
            error_log('1) $this->hijack_ui = false');
        }
        ...
        if($this->hijack_ui === true) {
            error_log('2) $this->hijack_ui = true');
        } else {
            error_log('2) $this->hijack_ui = false');
        }
    ...
    public function onRouterInitialized($m)
    {
        ...
        if($this->hijack_ui === true) {
            error_log('3) $this->hijack_ui = true');
        } else {
            error_log('3) $this->hijack_ui = false');
        }
        if ($this->hijack_ui === true) {
